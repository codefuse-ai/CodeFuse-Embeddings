...
def save_checkpoint(args, accelerator, model, output_dir, lr_scheduler):
    ...
    state_dict = accelerator.get_state_dict(model) if args.r > 0 else None
    if accelerator.is_main_process:
        model.tokenizer.save_pretrained(output_dir)
        if args.r > 0:
            torch.save(state_dict['U'], f"{output_dir}/U.pth")
            torch.save(state_dict['V'], f"{output_dir}/V.pth")
    unwrapped_model = accelerator.unwrap_model(model.lm)

    if args.r > 0:
        with torch.no_grad():
            embed_matrix = model.U @ model.V
        unwrapped_model.embed_tokens.weight = nn.Parameter(embed_matrix)
